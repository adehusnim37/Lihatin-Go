...
func RunCleanupScheduler(ctx context.Context, repo *repositories.LoginAttemptRepository, interval time.Duration, olderThanDays int) {
    ...
    utils.Logger.Info("LoginAttempt cleanup scheduler started", "interval", interval.String(), "older_than_days", olderThanDays)

    for {
        // run immediately on start (optional)
        if err := repo.CleanupOldAttempts(olderThanDays); err != nil {
            utils.Logger.Error("CleanupOldAttempts failed", "error", err.Error())
        } else {
            utils.Logger.Info("CleanupOldAttempts finished")
        }

        select {
        case <-ctx.Done():
            utils.Logger.Info("Cleanup scheduler stopped by context")
            return
        case <-ticker.C:
