...
func RegisterAuthRoutes(rg *gin.RouterGroup, authController *auth.Controller, userRepo userrepo.UserRepository, userAuthRepo *authrepo.UserAuthRepository, loginAttemptRepo authrepo.LoginAttemptRepository, emailController *email.Controller, totpController *totp.Controller, baseController *controllers.BaseController) {
	...
	protectedAuth.Use(middleware.AuthMiddleware(userRepo, userAuthRepo), middleware.RequireEmailVerification())
	{
		...
		protectedAuth.POST("/sessions/revoke-device", middleware.RateLimitMiddleware(10, 0, 10), authController.RevokeDevice)
		protectedAuth.GET("/sessions", middleware.RateLimitMiddleware(20, 0, 10), authController.ListSessions)
		protectedAuth.POST("/sessions/revoke", middleware.RateLimitMiddleware(10, 0, 10), authController.RevokeSession)
		protectedAuth.POST("/sessions/revoke-all", middleware.RateLimitMiddleware(10, 0, 10), authController.RevokeAllSessions)
	...
	adminAuth.Use(middleware.AuthMiddleware(userRepo, userAuthRepo), middleware.AdminAuth(), middleware.RequireEmailVerification())
	{
		...
		adminAuth.GET("/users/email-options", adminController.ListUserEmail)
