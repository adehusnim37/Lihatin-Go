...
func (es *EmailService) SendChangeOldEmailNotification(oldEmail, newEmail, username, revokeToken string) error {
	...
	revokeURL := fmt.Sprintf("%s/v1/auth/revoke-email-change?token=%s", utils.GetRequiredEnv(utils.EnvBackendURL), revokeToken)
...
`, username, oldEmail, newEmail, revokeURL, utils.GetRequiredEnv(utils.EnvBackendURL))
