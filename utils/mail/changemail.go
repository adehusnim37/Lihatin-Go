...
func (es *EmailService) SendChangeEmailConfirmation(toEmail, userName, token string) error {
	...
	confirmationURL := fmt.Sprintf(utils.GetRequiredEnv(utils.EnvBackendURL)+`/v1/auth/verify-email?token=%s`, token)
...
`, userName, confirmationURL, utils.GetRequiredEnv(utils.EnvBackendURL))
